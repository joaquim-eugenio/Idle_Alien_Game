...
export function useCamera(viewportWidth: number, viewportHeight: number): CameraState {
  ...
  const cachedRef = useRef<CameraState | null>(null);
  ...
  useEffect(() => {
    const now = Date.now();
    if (now - lastCalcRef.current >= CAMERA_THROTTLE_MS) {
      lastCalcRef.current = now;
      cachedRef.current = computed;
    } else if (cachedRef.current) {
      const timer = setTimeout(() => forceUpdate(n => n + 1), CAMERA_THROTTLE_MS);
      return () => clearTimeout(timer);
    }
  }, [computed]);

  return cachedRef.current ?? computed;
}
